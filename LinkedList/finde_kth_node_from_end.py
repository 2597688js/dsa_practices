...
File_name = finde_kth_node_from_end.py
Date : 23-01-2024
Description :
...
3. If the fast pointer becomes None before moving k nodes, the function should return None, as the list is shorter
 than k nodes.

4. The slow and fast pointers should then move forward in the list at the same time until the fast pointer reaches
 the end of the list.
...
def find_kth_from_end(ll, k):
    ...
    for _ in range(k):
        if fast is None:
            return None  # linked list is shorter than k
        fast = fast.next
